...
func TestStateMv_backupAndBackupOutFlagsWithNonLocalBackend(t *testing.T) {
	state := states.BuildState(func(s *states.SyncState) {
		s.SetResourceInstanceCurrent(
			addrs.Resource{
				Mode: addrs.ManagedResourceMode,
	...
	t.Run("backup flag specified", func(t *testing.T) {
		td := tempDir(t)
		...
		wantErr := `
Error: Invalid command-line flags: -backup

Command-line flags -backup and -backup-out are legacy options that operate on
a local state file only. You must specify a local state file with the -state
flag or switch to the local backend.
...
`
	...
	t.Run("backup-out flag specified", func(t *testing.T) {
		td := tempDir(t)
		testCopyDir(t, testFixturePath("init-backend-http"), td)
		defer os.RemoveAll(td)
		defer testChdir(t, td)()
		...
		gotErr := ui.ErrorWriter.String()
		wantErr := `
Error: Invalid command-line flags: -backup-out
...
`
	...
	t.Run("backup and backup-out flags specified", func(t *testing.T) {
		td := tempDir(t)
		testCopyDir(t, testFixturePath("init-backend-http"), td)
		defer os.RemoveAll(td)
		defer testChdir(t, td)()
		...
		gotErr := ui.ErrorWriter.String()
		wantErr := `
Error: Invalid command-line flags: -backup, -backup-out
...
`
	...
	t.Run("backup flag specified with state flag", func(t *testing.T) {
		td := tempDir(t)
		testCopyDir(t, testFixturePath("init-backend-http"), td)
		defer os.RemoveAll(td)
		defer testChdir(t, td)()
		...
		// Test it is correct
		testStateOutput(t, statePath, testStateMvBackupAndBackupOutFlagsWithNonLocalBackendOutput)
	})

	t.Run("backup-out flag specified with state flag", func(t *testing.T) {
		td := tempDir(t)
		testCopyDir(t, testFixturePath("init-backend-http"), td)
		defer os.RemoveAll(td)
		defer testChdir(t, td)()
...
const testStateMvBackupAndBackupOutFlagsWithNonLocalBackendOutput = `
test_instance.bar:
  ID = bar
  provider = provider["registry.terraform.io/hashicorp/test"]
  bar = value
